<script lang="ts">
	...
	async function aiGenerateCover() {
		...
		if (!title) { alert('Scrie titlul mai întâi.'); return; }
	...
	async function aiSummarize() {
		...
		const content = (document.querySelector('input[name="content"]') as HTMLInputElement)?.value
			|| (document.querySelector('textarea[name="content"]') as HTMLTextAreaElement)?.value;
		if (!content) { alert('Scrie conținutul mai întâi.'); return; }
		summarizing = true;
		try {
...
<div class="page">
	<div class="page-header">
		<div>
			<a href="/admin/studii-de-caz" class="back">← Înapoi la studii de caz</a>
			<h1>Studiu de Caz Nou</h1>
		</div>
		<div class="header-actions">
			<button type="submit" form="cs-form" class="btn-save" disabled={saving}>
	...
	<form id="cs-form" method="post" action="?/create" use:enhance={() => { saving = true; dirty = false; return async ({ update }) => { saving = false; await update(); }; }} class="article-form" oninput={() => dirty = true}>
		<input type="hidden" name="published" value={publishNow ? 'on' : ''} />
		<div class="form-grid">
			<div class="form-main">
				<div class="field">
					<label for="title">Titlu</label>
					<input type="text" id="title" name="title" required placeholder="Titlul studiului de caz" />
				</div>
				...
				<div class="field">
					...
					<textarea id="excerpt" name="excerpt" rows="3" placeholder="Scurt rezumat al studiului de caz"></textarea>
				</div>

				<div class="field">
					<!-- svelte-ignore a11y_label_has_associated_control -->
					<label>Conținut (Markdown)</label>
					<MarkdownEditor placeholder="Scrie studiul de caz aici folosind Markdown..." />
				</div>
			</div>
			...
			<div class="form-sidebar">
				<div class="sidebar-card">
					...
					<p class="info-hint">Salvează pentru a adăuga traduceri.</p>
				</div>
