<script lang="ts">
	...
	async function aiGenerateCover() {
		...
		if (!title) { alert('Scrie titlul studiului de caz mai întâi.'); return; }
	...
	async function aiSummarize() {
		...
		const content = (document.querySelector('input[name="content"]') as HTMLInputElement)?.value
			|| (document.querySelector('textarea[name="content"]') as HTMLTextAreaElement)?.value;
		if (!content) { alert('Scrie conținutul studiului de caz mai întâi.'); return; }
		summarizing = true;
		try {
...
<div class="page">
	...
	{#if form?.success}
		<div class="success">Studiul de caz a fost actualizat cu succes.</div>
	{/if}
		...
		<div class="form-grid">
			...
			<div class="form-sidebar">
				<div class="sidebar-card">
					...
					{#if a.slug}
						<a href="/studii-de-caz/{a.slug}" class="view-link" target="_blank">Vizualizează →</a>
					{/if}
				</div>
				...
				<div class="sidebar-card">
					<h3>Limba</h3>
					<div class="field">
						<label for="lang">Limba studiului de caz</label>
					...
					<div class="form-grid">
						...
						<div class="form-sidebar">
							<div class="sidebar-card">
								<h3>Info</h3>
								<p class="info-text">
									Categoria și imaginea sunt sincronizate cu studiul de caz sursă.
								</p>
							</div>
						</div>
					</div>
